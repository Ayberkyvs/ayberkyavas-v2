...
import MessageBubble from "../../assets/sayhi.png"
import { Link } from "react-router-dom";
export default function Sayhi({className}: {className?: string}) {
  return (
    <section className={`flex flex-col-reverse md:flex-row justify-center items-center gap-[25px] bg-hover-bg-1 border-[1px] shadow-lg border-nav-border rounded-xl p-[32px] ${className}`}>
        ...
        <div className="flex justify-center items-center w-auto h-[275px]">
            <img src={MessageBubble} alt="Message Bubbles" className="w-full h-full object-cover" draggable={false}/>
        </div>
    </section>
  )
}