...
export default function ProjectCard({align, data}: {align?: boolean, data: any}) {
  return (
    <a className={`relative flex w-full md:w-[224px] items-center md:flex-col rounded-[16px] bg-transparent hover:bg-gradient-to-r
     from-[#1E1C32]/50 to-[#2B161D]/50 md:min-h-[352px] h-fit p-[12px]
      text-primary-text ${align ? "flex-row-reverse" : ""} cursor-pointer`} href={data.link ? data.link : undefined} target={data.link ? "_blank": undefined}>
